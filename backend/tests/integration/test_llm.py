...
async def test_get_chat_completion_batch():
    ...
    with respx.mock:
        ...
        request = LLMRequest(messages=[ChatMessage(role="user", content="Hi")], stream=False)
...
async def test_get_chat_completion_stream():
    ...
    with respx.mock:
        ...
        respx.post("https://api.openai.com/v1/chat/completions").mock(
            return_value=Response(200, content=stream_gen(), headers={"Content-Type": "text/event-stream"})
        )
...
async def test_get_chat_completion_with_params():
    with respx.mock:
        ...
        request = LLMRequest(messages=[ChatMessage(role="user", content="Hi")], temperature=0.0, max_tokens=10)
...
async def test_openai_api_error_handling():
    with respx.mock:
        respx.post("https://api.openai.com/v1/chat/completions").mock(
            return_value=Response(401, json={"error": {"message": "Invalid API Key", "type": "invalid_request_error"}})
        )
